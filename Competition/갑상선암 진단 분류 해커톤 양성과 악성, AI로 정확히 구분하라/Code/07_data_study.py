...
for col in outlier_cols:
    ...
    # plt.figure(figsize=(7, 5)) # === 여기에 수정하세요 === 이 부분은 주석 처리 또는 제거
    # sns.boxplot(x=x[col])     # === 여기에 수정하세요 === 이 부분은 주석 처리 또는 제거
    # plt.title(f'{col} Boxplot (After Log, Before Winsorization)') # === 여기에 수정하세요 === 이 부분은 주석 처리 또는 제거
    # plt.show()                # === 여기에 수정하세요 === 이 부분은 주석 처리 또는 제거
    ...
    # plt.figure(figsize=(7, 5)) # === 여기에 수정하세요 === 이 부분은 주석 처리 또는 제거
    # sns.boxplot(x=x[col])     # === 여기에 수정하세요 === 이 부분은 주석 처리 또는 제거
    # plt.title(f'{col} Boxplot (After Log, After Winsorization)') # === 여기에 수정하세요 === 이 부분은 주석 처리 또는 제거
    # plt.show()                # === 여기에 수정하세요 === 이 부분은 주석 처리 또는 제거
...
# plt.figure(figsize=(8, 6)) # === 여기에 수정하세요 === 이 부분은 주석 처리 또는 제거
# sns.heatmap(corr_matrix, annot=True, cmap='coolwarm', fmt=".2f", linewidths=.5) # === 여기에 수정하세요 === 이 부분은 주석 처리 또는 제거
# plt.title('Correlation Matrix of Continuous Variables') # === 여기에 수정하세요 === 이 부분은 주석 처리 또는 제거
# plt.show() # === 여기에 수정하세요 === 이 부분은 주석 처리 또는 제거
...
# === 여기에 수정하세요 === 딥러닝 모델을 위해 양성 클래스(1)의 가중치를 수동으로 더 높여줌 (더 과감하게 조절)
# 예시: 3배로 증폭 (이전 2.5배에서 효과 미미했으므로 더 높여봅니다)
weight_multiplier = 2.5 # 이 값을 2.5, 3.0 등으로 조절하며 실험
...
model.fit(x_train_scaled, y_train_smote, # SMOTE가 적용된 데이터 사용
    epochs=400, batch_size=128, validation_split=0.2,
...
# === 여기에 수정하세요 === 딥러닝 모델 임계값 설정 (재현율 우선)
target_recall_threshold_dl = 0.55 # 딥러닝 모델의 목표 재현율 설정 (예: 70%)

valid_indices_dl = np.where(recall >= target_recall_threshold_dl)
if len(valid_indices_dl[0]) > 0:
    best_idx_for_recall_dl = valid_indices_dl[0][np.argmax(precision[valid_indices_dl])]
    best_thresh_dl = thresholds[best_idx_for_recall_dl]
else:
    # 목표 재현율을 달성하는 임계값이 없는 경우, F1-Score 최적 임계값 사용 (fallback)
    f1_scores = 2 * (precision * recall) / (precision + recall + 1e-8)
    best_idx_dl = np.argmax(f1_scores)
    best_thresh_dl = thresholds[best_idx_dl]

y_pred_dl = (y_proba_dl >= best_thresh_dl).astype(int)

print(f"DL Best Threshold (Recall goal {target_recall_threshold_dl}): {best_thresh_dl}")
print(f"DL Accuracy: {accuracy_score(y_test, y_pred_dl)}")
print(f"DL F1 Score: {f1_score(y_test, y_pred_dl)}")
# best_thresh_dl에 해당하는 Recall과 Precision 값을 정확히 가져옴
best_dl_recall_at_thresh = recall[np.where(thresholds == best_thresh_dl)[0][0]] if best_thresh_dl in thresholds else recall[best_idx_for_recall_dl]
best_dl_precision_at_thresh = precision[np.where(thresholds == best_thresh_dl)[0][0]] if best_thresh_dl in thresholds else precision[best_idx_for_recall_dl]

print(f"DL Recall: {best_dl_recall_at_thresh}")
print(f"DL Precision: {best_dl_precision_at_thresh}") # 정밀도도 같이 확인

# ================== 5. 부스팅 모델 ==================
# 양성 클래스 가중치 계산 (SMOTE 적용 후의 분포 기반)
# 0 클래스(음성) 개수 / 1 클래스(양성) 개수
num_neg_smote = y_train_smote.value_counts()[0]
num_pos_smote = y_train_smote.value_counts()[1]
# === 여기에 수정하세요 === 부스팅 모델 가중치 조정 (딥러닝과 동일한 weight_multiplier 적용)
...
xgb = XGBClassifier(
    ...
    random_state=42,
    scale_pos_weight=boosting_weight_for_pos # === 여기에 수정하세요 === 조정된 가중치 적용
)
...
lgb_model = lgb.LGBMClassifier(
    n_estimators=500, learning_rate=0.05, max_depth=6,
    # === 여기에 수정하세요 === LightGBM도 scale_pos_weight를 사용 (class_weight='balanced' 대신)
    scale_pos_weight=boosting_weight_for_pos, 
    random_state=42
)
...
cat_model = CatBoostClassifier(
    ...
    random_state=42,
    # === 여기에 수정하세요 === CatBoost의 class_weights 조정
    class_weights=[class_weight_dict_smote[0], class_weight_dict_smote[1]] # 딥러닝과 동일한 조정된 가중치 사용
)
...
# === 여기에 수정하세요 === 앙상블 임계값 설정 (재현율 우선)
precision, recall, thresholds = precision_recall_curve(y_test, y_proba_ensemble)
target_recall_threshold_ensemble = 0.60 # 앙상블 모델의 목표 재현율 설정 (예: 75%)
# 이 값은 0.7, 0.8 등으로 바꿔가며 실험해보세요.

valid_indices_ensemble = np.where(recall >= target_recall_threshold_ensemble)
if len(valid_indices_ensemble[0]) > 0:
    best_idx_for_recall_ensemble = valid_indices_ensemble[0][np.argmax(precision[valid_indices_ensemble])]
    best_thresh_ensemble = thresholds[best_idx_for_recall_ensemble]
else:
    # 목표 재현율을 달성하는 임계값이 없는 경우, F1-Score 최적 임계값 사용 (fallback)
    f1_scores = 2 * (precision * recall) / (precision + recall + 1e-8)
    best_idx_ensemble = np.argmax(f1_scores)
    best_thresh_ensemble = thresholds[best_idx_ensemble]

y_pred_ensemble = (y_proba_ensemble >= best_thresh_ensemble).astype(int)

print(f"Ensemble Best Threshold (Recall goal {target_recall_threshold_ensemble}): {best_thresh_ensemble}")
print(f"Ensemble Accuracy: {accuracy_score(y_test, y_pred_ensemble)}")
print(f"Ensemble F1 Score: {f1_score(y_test, y_pred_ensemble)}")

# best_thresh_ensemble에 해당하는 Recall과 Precision 값을 정확히 가져옴
best_ensemble_recall_at_thresh = recall[np.where(thresholds == best_thresh_ensemble)[0][0]] if best_thresh_ensemble in thresholds else recall[best_idx_for_recall_ensemble]
best_ensemble_precision_at_thresh = precision[np.where(thresholds == best_thresh_ensemble)[0][0]] if best_thresh_ensemble in thresholds else precision[best_idx_for_recall_ensemble]

print(f"Ensemble Recall: {best_ensemble_recall_at_thresh}")
print(f"Ensemble Precision: {best_ensemble_precision_at_thresh}") # 정밀도도 같이 확인
...
submission_csv.to_csv(path_1 + 'submission_2045_manual_ensemble_smote_recall_focused.csv') # 파일명 변경
print("✅ submission_2045_manual_ensemble_smote_recall_focused.csv 파일 생성 완료!")