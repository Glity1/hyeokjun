...
# --- 원본 데이터의 연속형 컬럼 통계량 확인 ---
print("##### 연속형 컬럼 원본 데이터 통계량 (처리 전) #####")
continuous_cols = ['Age', 'Nodule_Size', 'TSH_Result', 'T4_Result', 'T3_Result']
print(train_csv[continuous_cols].describe())
print("\n")
...
# 2-1. 분포 변환 (로그 변환)
log_transform_cols = ['Nodule_Size', 'TSH_Result', 'T4_Result', 'T3_Result']

print("##### 로그 변환 전후 Nodule_Size 시각화 및 통계량 #####")
# Nodule_Size 로그 변환 전 히스토그램 & 기술 통계량
plt.figure(figsize=(12, 5))
plt.subplot(1, 2, 1)
sns.histplot(x['Nodule_Size'], kde=True)
plt.title('Nodule_Size (Before Log Transform)')
plt.subplot(1, 2, 2)
sns.boxplot(x=x['Nodule_Size'])
plt.title('Nodule_Size Boxplot (Before Log Transform)')
plt.tight_layout()
plt.show()
print("Nodule_Size (Before Log Transform) describe:\n", x['Nodule_Size'].describe())


for col in log_transform_cols:
    x[col] = np.log1p(x[col])
    test_csv_processed[col] = np.log1p(test_csv_processed[col])
    # print(f"'{col}' 컬럼에 로그 변환 적용 완료.") # 너무 많이 출력될 수 있으니 주석 처리

# Nodule_Size 로그 변환 후 히스토그램 & 기술 통계량
print("\nNodule_Size (After Log Transform) describe:\n", x['Nodule_Size'].describe())
plt.figure(figsize=(12, 5))
plt.subplot(1, 2, 1)
sns.histplot(x['Nodule_Size'], kde=True)
plt.title('Nodule_Size (After Log Transform)')
plt.subplot(1, 2, 2)
sns.boxplot(x=x['Nodule_Size'])
plt.title('Nodule_Size Boxplot (After Log Transform)')
plt.tight_layout()
plt.show()
print("로그 변환으로 분포가 더 정규분포에 가까워졌는지 확인하세요.")
...
def handle_outliers_iqr(df, column, train_q1=None, train_q3=None):
    ...
    initial_outliers = df[(df[column] < lower_bound) | (df[column] > upper_bound)].shape[0]
    print(f"  컬럼 '{column}': 처리 전 이상치 개수 = {initial_outliers} (하한:{lower_bound:.2f}, 상한:{upper_bound:.2f})")
...
for col in outlier_cols:
    ...
    # 처리 전후 박스플롯 비교 (선택 사항 - 그래프가 많아질 수 있음)
    # plt.figure(figsize=(12, 4))
    # plt.subplot(1, 2, 1)
    # sns.boxplot(x=x[col])
    # plt.title(f'{col} Boxplot (After Log, Before Winsorization)')
    # plt.subplot(1, 2, 2)
    # sns.boxplot(x=x_after_outliers_processed[col]) # x_after_outliers_processed는 handle_outliers_iqr 결과라고 가정
...
# ================== 3. 부스팅 모델 ==================
xgb = XGBClassifier(n_estimators=500, learning_rate=0.05, max_depth=6, subsample=0.8, colsample_bytree=0.8, random_state=42, scale_pos_weight=class_weight_dict[0] / class_weight_dict[1]
)
...
lgb_model = lgb.LGBMClassifier(n_estimators=500, learning_rate=0.05, max_depth=6, class_weight='balanced', random_state=42
)
...
cat_model = CatBoostClassifier(iterations=500, learning_rate=0.05, depth=6, verbose=0,random_state=42, class_weights=[class_weight_dict[0], class_weight_dict[1]]
)
...
voting = VotingClassifier( estimators=[('xgb', xgb),('lgb', lgb_model),('cat', cat_model),],voting='soft')
