...
lasso.fit(maccs_data_scaled, y_valid)
coef_importance = np.abs(lasso.coef_)
top_30_maccs_indices = sorted(np.argsort(coef_importance)[-30:])
...
global_features_all = [get_rdkit_and_selected_maccs(s, top_30_maccs_indices) for s in smiles_train]
...
class GATv2WithGlobal_JK(Module):
    def __init__(self, node_feat_dim=9, edge_feat_dim=None, global_feat_dim=50, hidden_channels=128, heads=4, dropout=0.3):
        super().__init__()
...
def objective(trial):
    ...
    data_list = [
        smiles_to_graph_final(s, l, top_30_maccs_indices, scaler,
    ...
    model = GATv2WithGlobal_JK(hidden_channels=hidden_channels, heads=heads, dropout=dropout,
                                edge_feat_dim=edge_feat_dim, global_feat_dim=20 + len(top_30_maccs_indices)).to(device)
...
final_test_data = [
    smiles_to_graph_final(s, None, top_30_maccs_indices, scaler,
                          lambda b: combine_bond_features(b, bond_funcs), edge_feat_dim)
...
model = GATv2WithGlobal_JK(
    ...
    global_feat_dim=20 + len(top_30_maccs_indices)
).to(device)
...
for epoch in range(1, 101):
    ...
    if (score > best_score) or (rmse < best_rmse and corr > best_corr):
        best_score, best_rmse, best_corr = score, rmse, corr
        patience_counter = 0
        torch.save(model.state_dict(), './_save/gnn_optimized_model.pt')
    else:
        patience_counter += 1
...
# ✅ Test 예측 및 저장
model.load_state_dict(torch.load('./_save/gnn_optimized_model.pt'))
model.eval()
test_preds = []
with torch.no_grad():
...
submit_df.to_csv('./_save/gnn_optimized_submission.csv', index=False)
print("\n✅ 최종 제출 파일 저장 완료: ./_save/gnn_optimized_submission.csv")