...
# 모든 MACCS 키 데이터 추출 (최적화 실행 전에 MACCS 키를 미리 추출)
...
# `smiles_to_graph_final` 함수 수정: bond_feature_extractor 인자 추가
def smiles_to_graph_final(smiles, label=None, selected_maccs_indices=None, scaler=None, bond_feature_extractor=None, edge_feature_dim=None):
    ...
    if not edge_indices: # 분자에 결합이 없는 경우 (예: 단일 원자)
        ...
        edge_attr = torch.empty((0, edge_feature_dim), dtype=torch.float) # 동적으로 전달받은 차원 사용
    else:
...
# Global Scaler는 모든 `global_feat`에 대해 한 번만 fit
global_features_all = [
...
class GATv2WithGlobal(Module):
    def __init__(self, node_feat_dim=9, edge_feat_dim=None, global_feat_dim=global_feat_dim_updated, hidden_channels=128, heads=4, dropout=0.3):
        ...
        # --- 수정 부분 시작 ---
        # fc1의 in_features를 정확히 계산하여 오류 방지
    ...
    def forward(self, data):
        ...
        # global_feat의 배치 사이즈가 x의 배치 사이즈와 다른 경우
        if global_feat.size(0) != x.size(0):
             # 여기서 오류가 발생할 가능성이 높음. 배치 사이즈가 다른 경우.
             # 임시 방편으로 재배치
...
# ----------------------------------------------------
#               Optuna Objective 함수 (엣지 피처 선택 로직 추가)
# ----------------------------------------------------
...
def objective(trial):
    ...
    edge_feature_choice = trial.suggest_categorical('edge_feature_choice', ['basic', 'all_extended'])

    if edge_feature_choice == 'basic':
        current_bond_feature_extractor = bond_features_basic
        try:
            current_edge_feature_dim = len(bond_features_basic(Chem.MolFromSmiles('CC').GetBonds()[0]))
        except Exception:
            current_edge_feature_dim = 9
    elif edge_feature_choice == 'all_extended':
        current_bond_feature_extractor = bond_features_all_extended
        try:
            current_edge_feature_dim = len(bond_features_all_extended(Chem.MolFromSmiles('CC').GetBonds()[0]))
        except Exception:
            current_edge_feature_dim = 17

    print(f"\nOptuna Trial {trial.number}: Using edge feature choice '{edge_feature_choice}' (dim: {current_edge_feature_dim})")

    # Optuna Trial을 위한 데이터셋 새로 생성
...
if final_edge_feature_choice == 'basic':
    final_bond_feature_extractor = bond_features_basic
    final_edge_feature_dim = len(bond_features_basic(Chem.MolFromSmiles('CC').GetBonds()[0]))
elif final_edge_feature_choice == 'all_extended':
    final_bond_feature_extractor = bond_features_all_extended
    final_edge_feature_dim = len(bond_features_all_extended(Chem.MolFromSmiles('CC').GetBonds()[0]))
else:
    print(f"WARNING: Unknown best edge feature choice '{final_edge_feature_choice}'. Defaulting to 'all_extended'.")
    final_bond_feature_extractor = bond_features_all_extended
    final_edge_feature_dim = len(bond_features_all_extended(Chem.MolFromSmiles('CC').GetBonds()[0]))
