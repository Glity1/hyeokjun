...
- + Safe patches:
  1) '신학기' 드라이버 & '캠퍼스/강의' 라이프스타일 계절 강화
  2) '기숙사/자취', '1–2인/소용량' 소가구 매핑 (family_mult)
  3) '유당 민감/소화 이슈=있음' → '소화가 잘되는 우유' SKU 선호 가중
  4) 소/대용량 SKU 분배(카테고리 합 보존)
...
class CONFIG:
    ...
    USE_POS_MONTH_SHARE = True
    POS_SEASON_START = 7
    POS_LAM_UNI      = 0.20
    POS_MIN_MSHARE   = 0.005

    # --- Custom month-share direct injection ---
    USE_CUSTOM_MONTH_SHARE = True
    CUSTOM_SHARE_WEIGHT    = 1.00  # 1.0=완전 치환, 0.5=절충
    CUSTOM_MONTH_SHARE_PATH = ""   # CSV(long: key,ym,share | wide: key,m1..m12)
    ...
    # --- NEW: Small/Large pack split ---
    USE_SMALLPACK_SPLIT = True
    SMALLPACK_STRENGTH  = 0.20   # 0~0.4 권장 (카테고리 합 보존 조건에서 소/대 조정 강도)
...
# --- PATCH #2: small household mapping added ---
def family_mult(v: object) -> float:
    s = str(v)
    # 소가구/소용량 선호(기숙사/자취, 1–2인/소용량 표기 포함)
    if any(k in s for k in ["기숙사", "자취", "1–2인", "1-2인", "1~2인", "소용량"]):
        return 0.90
...
# --- PATCH #1: campus/new semester seasonality added ---
def lifestyle_month_boost(month_int: int, lifestyle_text: object) -> float:
    ...
    # NEW: 캠퍼스/강의 중심 → 학기중(3-4, 9-10월) 가중
    if "캠퍼스" in s or "강의" in s:
        if m in [3,4,9,10]: mult *= 1.05
    return mult
...
def smooth_event_boost(month_int: int, drivers: object) -> float:
    ...
    # NEW: 신학기 피크(3-4월 강, 9-10월 약)
    if any("신학기" in s for s in norm):
        mult *= gauss(3, m, sigma=0.8, peak=1.08)
        mult *= gauss(9, m, sigma=0.8, peak=1.03)
    return mult
...
def persona_base_for_month(p: dict, product_name: str, month_idx: int, month_int: int) -> float:
    ...
    # --- PATCH #3: 유당 민감 시 '소화가 잘되는 우유' SKU 가중 ---
    lacto_v, _ = get_vw(p, ["유당 민감/소화 이슈"])
    lacto_mult = 1.08 if ("있음" in str(lacto_v) and "소화가 잘되는 우유" in product_name) else 1.0

    mult = fam * loy * inc * hea * pro * life * rep * eventf * lacto_mult
    return base * mult
...
# =====================
# PATCH #4: Small/Large pack split (category total preserved)
# =====================
_SMALL_THRESH = {
    "참치캔": 100,      # <=100g small (90 vs 135)
    "식육가공품": 300,  # <=300g small (200 vs 340)
    "조미료": 700,      # <=700g small (500 vs 900)
    # 조제커피/발효유: 중립 처리(동일 용량 or 단일 SKU)
}

def _extract_size_g(name: str) -> Optional[float]:
    m = re.search(r"(\d+)\s*(g|G|ml|mL|ML)", name)
    if not m: return None
    val = float(m.group(1))
    unit = m.group(2).lower()
    # mL를 g로 환산하지 않고 그대로 비교(임계값들은 g 기준 정의)
    return val

def _smallpack_flag(product_name: str) -> str:
    cat = PROD2CAT.get(product_name, "")
    thr = _SMALL_THRESH.get(cat)
    if thr is None:
        return "neutral"
    sz = _extract_size_g(product_name)
    if sz is None:
        return "neutral"
    return "small" if sz <= thr else "large"

def _small_pref_score_for_category(personas: Dict[str, List[dict]], category: str) -> float:
    prods = CAT2PRODS.get(category, [])
    num = 0.0; den = 0.0
    for p_name in prods:
        for p in personas.get(p_name, []):
            val, w = get_vw(p, ["가구-용량 적합도"])
            pp = safe_float(p.get("purchase_probability", p.get("purchase_prob", 60)), 60)/100.0
            # 소용량 신호: 1–2인, 1-2인, 1~2인, 소용량, 기숙사, 자취
            is_small = 1.0 if any(k in str(val) for k in ["1–2인","1-2인","1~2인","소용량","기숙사","자취"]) else 0.0
            weight = (0.5 + 0.5*safe_float(w,0.6)) * pp
            num += is_small * weight
            den += weight
    if den <= 0:
        return 0.5
...
def apply_smallpack_split(pred_df: pd.DataFrame, personas: Dict[str, List[dict]]) -> pd.DataFrame:
    """카테고리별 소/대 용량 선호를 반영해 SKU 비중 조정(월별/카테고리 합 보존)."""
    if not CONFIG.USE_SMALLPACK_SPLIT:
        return pred_df
    base_col = "quantity_calib" if "quantity_calib" in pred_df.columns else "quantity"
    out = []
    # 월별, 카테고리별로 합 보존
    for d, gdate in pred_df.groupby("date", group_keys=False):
        for cat, prods in CAT2PRODS.items():
            gc = gdate[gdate["product_name"].isin(prods)].copy()
            if gc.empty:
                continue
            # 소/대 구분 존재할 때만 적용
            flags = gc["product_name"].map(_smallpack_flag)
            if not (flags.eq("small").any() and flags.eq("large").any()):
                out.append(gc)
                continue
            pref = _small_pref_score_for_category(personas, cat)  # 0~1
            bias = CONFIG.SMALLPACK_STRENGTH * (pref - 0.5) * 2.0  # -S..+S
            w = []
            for pn in gc["product_name"]:
                f = _smallpack_flag(pn)
                if f == "small": w.append(1.0 + bias)
                elif f == "large": w.append(1.0 - bias)
                else: w.append(1.0)
            w = np.array(w, float)
            w = np.clip(w, 0.2, 2.0)  # 안전 장치
            q = gc[base_col].to_numpy(float)
            q_new = q * w
            s = q_new.sum()
            if s > 0:
                q_new = q_new * (q.sum() / s)  # 카테고리 합 보존
            gc[base_col] = q_new
            out.append(gc)
...
def run():
    ...
    # --- PATCH #4: 소/대용량 분배(카테고리 합 보존) ---
    pred_df = apply_smallpack_split(pred_df, personas)
