...
from sklearn.linear_model import Lasso  # RandomForestRegressor 대신 Lasso 사용
...
print(f"Lasso 모델 학습 중... (MACCS 데이터 shape: {maccs_data.shape})")

# 데이터 스케일링
maccs_scaler = StandardScaler()
maccs_data_scaled = maccs_scaler.fit_transform(maccs_data)

lasso_model = Lasso(alpha=0.01, random_state=seed) # alpha 값은 조정 가능
lasso_model.fit(maccs_data_scaled, valid_y)

# 계수(coefficients)의 절대값으로 중요도 판단
...
print(f"\n✅ Lasso 계수 중요도로 선택된 Top 30 MACCS Key 인덱스 (0-indexed): {top_30_maccs_indices}")
...
def objective(trial):
    ...
    loss_fn = torch.nn.MSELoss() # MSELoss로 변경
...
for epoch in range(1, 101):
    ...
    if current_val_score > best_val_score:
        ...
        patience_counter = 0
        torch.save(model.state_dict(), f'./_save/gnn_lasso_top30_best_model.pt')
...
final_model_path = './_save/gnn_lasso_top30_best_model.pt'
...
submit_df.to_csv('./_save/gnn_lasso_top30_submission.csv', index=False)

print("\n✅ 최종 예측 완료: ./_save/gnn_lasso_top30_submission.csv")
...
# --- Optuna 최적화 결과 ---
# 최적 하이퍼파라미터: {'lr': 0.0035503404158060913, 'batch_size': 64, 'dropout': 0.10676296157459007, 'hidden_channels': 128, 'heads': 2, 'edge_feature_choice': 'ring_info'}
# 최고 Score: 0.6671353344184321

# ✅ 최종 예측 완료: ./_save/gnn_lasso_top30_submission.csv
#   사용된 MACCS 키 개수: 30
#   최종 global_feat 차원: 50
#   최종 엣지 피처 차원: 14 (선택: ring_info)