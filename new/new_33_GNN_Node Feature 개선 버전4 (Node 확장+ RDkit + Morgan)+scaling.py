...
def extract_features(smiles_list):
    rdkit_feats, morgan_feats = [], []
    for s in smiles_list:
        mol = Chem.MolFromSmiles(s)
        if mol is None:
            rdkit_feats.append(None)
            morgan_feats.append(None)
            continue
        try:
            ...
            morgan_feats.append(morgan_arr.astype(np.float32))
        except:
            rdkit_feats.append(None)
            morgan_feats.append(None)
    return rdkit_feats, morgan_feats

rdkit_train, morgan_train = extract_features(smiles_train)
rdkit_test, morgan_test = extract_features(smiles_test)
...
# RDKit + Morgan 결합
global_feats_train = [np.concatenate([r, m]) if r is not None and m is not None else None for r, m in zip(scaled_rdkit_train, morgan_train)]
global_feats_test = [np.concatenate([r, m]) if r is not None and m is not None else None for r, m in zip(scaled_rdkit_test, morgan_test)]
...
class GATv2Hybrid(Module):
    def __init__(self, node_feat_dim=13, global_feat_dim=2048+20):
        super().__init__()
...
for epoch in range(1, 101):
    ...
    if score > best_score:
        ...
        counter = 0
        torch.save(model.state_dict(), './_save/gnn_rdkit_morgan.pt')
    else:
        counter += 1
        if counter >= patience:
            print("EarlyStopping")
...
# 예측 및 저장
model.load_state_dict(torch.load('./_save/gnn_rdkit_morgan.pt'))
model.eval()
test_preds = []
with torch.no_grad():
...
submit_df.to_csv('./_save/gnn_rdkit_morgan_submission.csv', index=False)

print("\n🎯 [최종 결과 - 저장된 Best Model 기준]")
print(f"Best Score : {best_score:.4f}")
print(f"Best RMSE  : {best_rmse:.4f}")
print(f"Best Corr  : {best_corr:.4f}")
print("\n✅ 저장 완료: ./_save/gnn_rdkit_morgan_submission.csv")
