...
from sklearn.preprocessing import StandardScaler # ⭐️ StandardScaler 임포트
...
class Config:
    ...
    batch_size = 64
    learning_rate = 0.0005 # ⭐️ 이전 실험에서 조정한 학습률
    patience = 10 # ⭐️ 이전 실험에서 조정한 patience
    epochs = 100 # 최대 에포크 수
    ...
    # Global feature 관련
    # ⭐️ RDKit Descriptors 20개
    rdkit_desc_dim = 20
    # ⭐️ MACCSKeys 167개
    maccs_keys_dim = 167
    # ⭐️ 전체 글로벌 피처의 최종 차원 계산 (스케일링 전)
    global_feat_base_dim = rdkit_desc_dim + maccs_keys_dim
...
y_original = train_df['Inhibition'].values
...
y = y[valid_train_idx]
y_original = y_original[valid_train_idx]
...
test_original_indices = [test_df.index[i] for i in valid_test_smiles_original_idx]
...
def get_rdkit_and_maccs(smiles):
    ...
    try:
        ...
        desc = np.nan_to_num(desc, nan=0.0).astype(np.float32) # RDKit Descriptors (20개)
        maccs = MACCSkeys.GenMACCSKeys(mol)
        maccs_arr = np.zeros((Config.maccs_keys_dim,), dtype=np.int8) # MACCSKeys (167개)
        DataStructs.ConvertToNumpyArray(maccs, maccs_arr)
        return desc, maccs_arr.astype(np.float32) # ⭐️ RDKit Descriptors와 MACCSKeys 분리하여 반환
    except Exception as e:
...
# ⭐️ 변경된 부분: 원본 피처 데이터를 임시로 저장할 리스트
train_rdkit_descs_raw, train_maccs_keys_raw, train_morgan_top_raw, train_y_raw = [], [], [], []
train_smiles_filtered = []

# Train 데이터 처리
for i, s in enumerate(tqdm(smiles_train, desc="Processing Train Data")):
    rdkit_desc, maccs_keys = get_rdkit_and_maccs(s) # ⭐️ 분리된 형태로 받음
    if rdkit_desc is None: # 둘 중 하나라도 None이면 건너뜀
        continue
    
    # ⭐️ 차원 검증: RDKit Descriptors와 MACCS Keys 차원
    if len(rdkit_desc) != Config.rdkit_desc_dim or len(maccs_keys) != Config.maccs_keys_dim:
        print(f"오류: 트레인 데이터 {s}의 RDKit/MACCS 피처 차원 불일치. 예상 RDKit: {Config.rdkit_desc_dim}, 실제: {len(rdkit_desc)}. 예상 MACCS: {Config.maccs_keys_dim}, 실제: {len(maccs_keys)}")
        continue

    # ⭐️ 각 원본 피처를 리스트에 추가
    train_rdkit_descs_raw.append(rdkit_desc)
    train_maccs_keys_raw.append(maccs_keys)
    train_morgan_top_raw.append(morgan_train_top[i]) # Morgan FP는 이미 numpy 배열로 있음
    train_y_raw.append(y[i])
    train_smiles_filtered.append(s) # 유효한 SMILES만 저장

# Test 데이터 처리도 유사하게 분리
test_rdkit_descs_raw, test_maccs_keys_raw, test_morgan_top_raw, test_original_indices_filtered = [], [], [], []
test_smiles_filtered_for_processing = [] # 실제 처리되는 Test SMILES

current_morgan_test_idx = 0
for i, s_original in enumerate(test_df['Canonical_Smiles'].tolist()):
    if s_original in smiles_test_filtered: # morgan_test_raw에서 유효했던 SMILES만 처리
        test_smiles_filtered_for_processing.append(s_original)
        
        rdkit_desc, maccs_keys = get_rdkit_and_maccs(s_original) # ⭐️ 분리된 형태로 받음
        if rdkit_desc is None:
            current_morgan_test_idx += 1 # 이 SMILES에 대한 Morgan FP도 건너뛰도록 인덱스 증가
            continue

        # ⭐️ 차원 검증: RDKit Descriptors와 MACCS Keys 차원
        if len(rdkit_desc) != Config.rdkit_desc_dim or len(maccs_keys) != Config.maccs_keys_dim:
            print(f"오류: 테스트 데이터 {s_original}의 RDKit/MACCS 피처 차원 불일치. 예상 RDKit: {Config.rdkit_desc_dim}, 실제: {len(rdkit_desc)}. 예상 MACCS: {Config.maccs_keys_dim}, 실제: {len(maccs_keys)}")
            current_morgan_test_idx += 1
            continue

        if current_morgan_test_idx >= len(morgan_test_top):
            print(f"오류: Test Morgan FP 인덱스 초과. SMILES: {s_original}")
            continue

        test_rdkit_descs_raw.append(rdkit_desc)
        test_maccs_keys_raw.append(maccs_keys)
        test_morgan_top_raw.append(morgan_test_top[current_morgan_test_idx])
        test_original_indices_filtered.append(test_original_indices[i]) # ⭐️ 원래 인덱스 저장
        current_morgan_test_idx += 1


# ⭐️ RDKit Descriptors만 StandardScaler 적용
...
# 테스트 데이터 변환 (학습된 스케일러 사용)
test_rdkit_descs_scaled = scaler.transform(test_rdkit_descs_np)

# 이제 실제 Graph Data 객체 생성
train_data = []
for i in tqdm(range(len(train_rdkit_descs_scaled)), desc="Creating Train Graphs"):
    # ⭐️ 스케일링된 RDKit Descriptors, 스케일링되지 않은 MACCS Keys, Morgan TopN 결합
    global_feat_combined = np.concatenate([
        train_rdkit_descs_scaled[i],
        train_maccs_keys_raw[i], # MACCS Keys는 스케일링 안 함
        train_morgan_top_raw[i] # Morgan TopN은 스케일링 안 함
    ])
    
    # ⭐️ 최종 global_feat_dim 차원 확인
    expected_global_dim = Config.rdkit_desc_dim + Config.maccs_keys_dim + Config.use_topN
    if global_feat_combined.shape[0] != expected_global_dim:
        print(f"오류: 최종 글로벌 피처 차원 불일치. 예상: {expected_global_dim}, 실제: {global_feat_combined.shape[0]}")
        continue # 오류 발생 시 해당 샘플 건너뛰기

    data_obj = smiles_to_graph_with_global(train_smiles_filtered[i], global_feat_combined, train_y_raw[i])
    if data_obj is not None:
        train_data.append(data_obj)

test_data_with_indices = []
for i in tqdm(range(len(test_rdkit_descs_scaled)), desc="Creating Test Graphs"):
    global_feat_combined = np.concatenate([
        test_rdkit_descs_scaled[i],
        test_maccs_keys_raw[i], # MACCS Keys는 스케일링 안 함
        test_morgan_top_raw[i] # Morgan TopN은 스케일링 안 함
    ])
    ...
    graph_data = smiles_to_graph_with_global(test_smiles_filtered_for_processing[i], global_feat_combined)
    if graph_data is not None:
        test_data_with_indices.append((graph_data, test_original_indices_filtered[i]))


train_data = [d for d in train_data if d is not None] # 최종 None 필터링
...
class GATv2WithGlobal(Module):
    # ⭐️ global_feat_dim 계산 방식 변경: 스케일링 이전의 총 차원 그대로 사용
    def __init__(self, node_feat_dim=Config.node_feat_dim,
                 global_feat_dim=Config.rdkit_desc_dim + Config.maccs_keys_dim + Config.use_topN, # 변경된 부분
                 edge_feat_dim=Config.edge_feat_dim):
        super().__init__()
        # ⭐️ 변경된 부분: out_channels 증가
        self.gat1 = GATv2Conv(node_feat_dim, 128, heads=4, dropout=0.2, edge_dim=edge_feat_dim)
        self.bn1 = BatchNorm1d(128 * 4) # 128 * heads
        self.gat2 = GATv2Conv(128 * 4, 256, heads=4, dropout=0.2, edge_dim=edge_feat_dim) # 이전 레이어 출력 * heads를 입력으로
        self.bn2 = BatchNorm1d(256 * 4) # 256 * heads
        self.relu = ReLU()
        self.dropout = Dropout(0.3)
        # ⭐️ 변경된 부분: fc1의 입력 차원 조정
        self.fc1 = Linear(256 * 4 + global_feat_dim, 128) # global_mean_pool의 출력 차원 + global_feat_dim
...
model_save_path = f'./_save/gnn_edge{Config.edge_feat_dim}_stereo1hot_morgan{Config.use_topN}_log1p_scaled_rdkit_larger_gnn.pt'
...
val_trues_at_best_score_original_scale = None
...
for epoch in range(1, Config.epochs + 1):
    ...
    model.eval()
    preds_list_transformed, trues_list_transformed = [], []
    with torch.no_grad():
        for batch in tqdm(val_loader, desc=f"Epoch {epoch} Validation"):
            batch = batch.to(device)
            out = model(batch)
            preds_list_transformed.append(out.cpu().numpy())
            trues_list_transformed.append(batch.y.view(-1).cpu().numpy())
...
plt.title('검증 RMSE 및 상관계수 변화 (원본 스케일)', fontsize=14)
plt.xlabel('에포크', fontsize=12)
...
plt.tight_layout()
plt.savefig('./_save/training_metrics_curves_log1p_scaled_rdkit.png', dpi=300) # ⭐️ 파일명 변경
print(f"✅ 학습 지표 곡선 저장 완료: ./_save/training_metrics_curves_log1p_scaled_rdkit.png")
plt.show()
...
if val_preds_at_best_score is not None and val_trues_at_best_score_original_scale is not None:
    ...
    plt.title('검증 세트: 실제값 vs. 예측값 (Best Score 시점, 원본 스케일)', fontsize=16)
    plt.xlabel('실제 Inhibition 값', fontsize=14)
    plt.ylabel('예측 Inhibition 값', fontsize=14)
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend(fontsize=12)
    plt.gca().set_aspect('equal', adjustable='box')
    plt.savefig('./_save/validation_scatter_plot_best_score_log1p_scaled_rdkit.png', dpi=300) # ⭐️ 파일명 변경
    print(f"✅ 검증 세트 산점도 저장 완료: ./_save/validation_scatter_plot_best_score_log1p_scaled_rdkit.png")
    plt.show()
...
if val_preds_at_best_score is not None and val_trues_at_best_score_original_scale is not None:
    ...
    plt.title('검증 세트: 실제값 및 예측값 분포 (Best Score 시점, 원본 스케일)', fontsize=16)
    plt.xlabel('Inhibition 값', fontsize=14)
    plt.ylabel('빈도수', fontsize=14)
    plt.legend(fontsize=12)
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.savefig('./_save/validation_distribution_hist_best_score_log1p_scaled_rdkit.png', dpi=300) # ⭐️ 파일명 변경
    print(f"✅ 검증 세트 분포 히스토그램 저장 완료: ./_save/validation_distribution_hist_best_score_log1p_scaled_rdkit.png")
    plt.show()
...
submission_file_name = f'./_save/gnn_edge{Config.edge_feat_dim}_stereo1hot_morgan{Config.use_topN}_log1p_scaled_rdkit_larger_gnn_submission.csv'
