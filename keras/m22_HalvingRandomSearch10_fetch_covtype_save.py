...
model = HalvingGridSearchCV(xgb, parameters, cv=kfold,   
                     ...
                     factor=4,           # 배수 : min_resources * factor //  # 데이터가 크면 factor를 크게 조절 //  가급적이면 데이터전체를 훈련시키게끔 구조를 만든다
                                         #        n_candidates / factor   
                     min_resources=7262,   # 1 iter때의 최소 훈련 행의 개수
                     max_resources=464800,  # 데이터 행의 개수(n_samples)
                     )  
...
pd.DataFrame(model.cv_results_).sort_values(                    
     'rank_test_score',                                         
      ascending=True).to_csv(path + 'm18_10_rs_cv_results2.csv')

path = './_save/m15_cv_results/'
joblib.dump(model.best_estimator_, path + 'm18_10_best_model2.joblib' )
