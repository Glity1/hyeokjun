...
model = HalvingRandomSearchCV(xgb, parameters, cv=kfold,   
                     ...
                     max_resources=150,  # 데이터 행의 개수(n_samples)
                     )  
...
pd.DataFrame(model.cv_results_).sort_values(                    
     'rank_test_score',                                         
      ascending=True).to_csv(path + 'm18_00_rs_cv_results3.csv')

path = './_save/m15_cv_results/'
joblib.dump(model.best_estimator_, path + 'm18_00_best_model3.joblib' )