...
es = EarlyStopping(                    # EarlyStopping을 es라는 변수에 넣는다
    ...
    patience=400,                      # patience이 작으면 지역최소에 빠질수있음.  (history상에 10번 참는다는것은 마지막값에서 11번째 값이 최소값으로 보여준다.)
...
hist = model.fit(x,y, epochs=2000, batch_size=16,      
          verbose=1,                                  
          validation_split=0.2,
          callbacks=[es], 
          val_loss = 1413.3099365234375,
          
hist = model.fit(x,y, epochs=2000, batch_size=16,      
          verbose=1,                                  
          validation_split=0.2,
          callbacks=[es], 
          val_loss = 1493.7381591796875
          
          patience=50, 
          
hist = model.fit(x,y, epochs=2000, batch_size=2,      
          verbose=1,                                  
          validation_split=0.2,
          callbacks=[es], 
          val_loss = 1386.1552734375
