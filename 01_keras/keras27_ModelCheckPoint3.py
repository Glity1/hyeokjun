...
mcp=ModelCheckpoint(
    ...
    filepath=path+'keras27_mcp3.hdf5'   # 과적합이 되기 전 가장 일반화 성능이 좋았던 모델을 저장하기 위함.
)
...
path='./_save/keras27_mcp/'
model.save(path+'keras27_3_save_model.h5') # 최종 훈련 완료 모델을 그대로 저장 → 나중에 load_model()로 불러와 바로 평가·예측 가능.
