...
#                      random_state=333,
#                      n_iter=42,        # 경우의 수 만큼까지 할 수 있다. 42까지 다하면 gridsearch랑 동일함.
#                      )  
...
model = HalvingGridSearchCV(xgb, parameters, cv=kfold,   
                     ...
                     random_state=333,   
                     # default
                     factor=3,           # 배수 : min_resources * factor //  # 데이터가 크면 factor를 크게 조절 //  가급적이면 데이터전체를 훈련시키게끔 구조를 만든다
                                         #        n_candidates / factor   
...
pd.DataFrame(model.cv_results_).sort_values(                    
     'rank_test_score',                                         
      ascending=True).to_csv(path + 'm18_00_rs_cv_results2.csv')

path = './_save/m15_cv_results/'
joblib.dump(model.best_estimator_, path + 'm18_00_best_model2.joblib' )
